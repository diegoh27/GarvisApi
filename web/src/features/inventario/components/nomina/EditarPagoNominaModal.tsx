...
import { useUpdatePagoNominaMutation } from "../../api/nominaApi";
import { MONTO_MIN, MONTO_MAX, sanitizeMonto, validarMonto } from "../../utils/validation";
import type { NominaPago, UpdatePagoNominaPayload } from "../../api/nominaApi";

interface EditarPagoNominaModalProps {
  isOpen: boolean;
...
export default function EditarPagoNominaModal({
  isOpen,
  pago,
  onClose,
  onSuccess,
}: EditarPagoNominaModalProps) {
  const [formData, setFormData] = useState<UpdatePagoNominaPayload>({
    fecha_pago: "",
    fecha_proximo_pago: "",
    monto: 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.fecha_proximo_pago === "") {
      setError("La fecha prxima es requerida");
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              disabled
              value={`${pago.nombre_empleado} ${pago.apellido || ""}`}

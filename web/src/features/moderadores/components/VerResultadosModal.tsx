...
const VerResultadosModal = ({
	...
	// Actualizar archivos cuando cambian los props
	useEffect(() => {
	...
	const handleDeleteArchivo = async (url: string) => {
		...
		try {
			...
			// Llamar al callback para refrescar los datos en la página principal
			if (onArchivoDeleted) {
				await onArchivoDeleted();
			}

			// Si no quedan archivos, cerrar el modal después de un breve delay
			...
			Swal.fire({
				icon: "error",
				title: "Error",
				text: error?.data?.message || "No se pudo eliminar el archivo",
			});
		}
	};
							...
							return (
								...
								>
									...
									<div className="ml-4 flex items-center gap-2 flex-shrink-0">
										...
										<button
											type="button"
											onClick={() => handleDeleteArchivo(url)}
											disabled={isDeleting}
											className="rounded-lg border border-red-500 bg-paper px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
